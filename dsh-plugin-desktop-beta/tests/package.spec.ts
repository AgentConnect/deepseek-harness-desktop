...
const runtimeVersion = '0.1.5-alpha.2'
...
describe('published package surface', () => {
  ...
  it('patches the browse panel with the Windows native-picker icon bridge', () => {
    const patchPath = './patches/dsh-client-ui-directory-picker-browse@0.1.5-alpha.2.patch'
  ...
  it('patches the browse backend to skip unreadable directory-looking entries', () => {
    const patchPath = './patches/dsh-host-directory-picker-browse@0.1.5-alpha.2.patch'
  ...
  it('gives the Desktop settings section a dedicated display icon', () => {
    const patchPath = './patches/dsh-client-ui-settings-general@0.1.5-alpha.2.patch'
  ...
  it('keeps wide Markdown table scrollbars visible without hover', () => {
    const patchPath = './patches/dsh-client-ui-primitives@0.1.5-alpha.2.patch'
  ...
  it('retains the pre-alpha.2 settings helpers used by profile plugins', () => {
    const patchPath = './patches/dsh-settings@0.1.5-alpha.2.patch'
  ...
  it('patches alpha app boot to keep packaged Profile fallbacks resolver-owned', () => {
    const patchPath = './patches/dsh-app-boot@0.1.5-alpha.2.patch'
  ...
  it('keeps the canonical web profile configurable while Desktop disables browser opening', () => {
    const patchPath = './patches/dsh-web-app@0.1.5-alpha.2.patch'
  ...
  it('hides official plugin-manager and general subprocess consoles on Windows', () => {
    const dshPatchPath = './patches/dsh@0.1.5-alpha.2.patch'
    const retiredSubprocessPatchPath = './patches/dsh-subprocess-local@0.1.5-alpha.2.patch'
    const lockfile = readFileSync(new URL('yarn.lock', workspaceRoot), 'utf8')
  ...
  it('starts restricted Windows shells with a hidden console show state', () => {
    const patchPath = './patches/dsh-win32-process@0.1.5-alpha.2.patch'
    const lockfile = readFileSync(new URL('yarn.lock', workspaceRoot), 'utf8')
    const patch = readFileSync(new URL('patches/dsh-win32-process@0.1.5-alpha.2.patch', workspaceRoot), 'utf8')
    const workspaceRequire = createRequire(new URL('package.json', packageRoot))
