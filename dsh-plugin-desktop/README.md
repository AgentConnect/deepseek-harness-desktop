...
Packaged applications check `https://awiki.me/downloads/dsh-awiki/stable/desktop-release.json` after 60 seconds, then every six hours. Settings and the tray share a single version request with a 15-second deadline. This AWiki distribution accepts its own canonical SemVer manifest: prerelease users can move to a newer prerelease or stable release; stable users are never offered prereleases. Background checks issue a lightweight notification once per new version. Failures retain the last successful in-memory result with an explicit failed-check state.

**Check for Updates…** shows a native result dialog with the fixed [Desktop download page](https://awiki.me/downloads/dsh-awiki/) even if discovery fails. Users download, install and restart manually. The client never downloads an installer, starts it, or deletes old installers. Existing installer cleanup receipts are left untouched. The public `desktopDistribution` service exposes only version, channel, download-page and check-state metadata; the AWiki settings page consumes it independently of active-tenant plugin compatibility requirements.

Release operators prepare all three platform artifacts and publish the matching manifest and download page together using `awiki-web/deploy/prepare-desktop-downloads.mjs`. The manifest identifies `awiki-dsh-desktop`; optional `bundled_versions.plugin/model_proxy` must describe the actual bundled runtime inputs. The checker accepts legacy manifests without that ID only from this fixed distribution endpoint. Independently published npm plugin versions are managed by each tenant's user-service policy, not by this Desktop manifest. Do not advertise runtime-input archives as public npm packages.
...
- Desktop upgrades use a download page and manual installation. Signed artifacts, publisher verification and native installation testing remain release gates.
