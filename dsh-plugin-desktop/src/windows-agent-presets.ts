...
export class WindowsAgentPresets extends AgentPresets {
  ...
  override async resolve(id?: string): Promise<AgentPreset> {
    ...
    throw new RemoteError('agent-preset/not-found', `Preset ${id} is unavailable on Windows`, { agentPreset: id, available: (await this.list()).map(candidate => candidate.id) })
  }
  ...
  override async copy(from: string, id: string, name?: string): Promise<void> {
    if (id === WINDOWS_UNSUPPORTED_PRESET) throw new RemoteError('agent-preset/invalid', `Preset ${id} is reserved on Windows`, { agentPreset: id, reason: 'reserved Windows preset' })
