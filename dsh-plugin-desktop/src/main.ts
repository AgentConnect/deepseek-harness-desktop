...
async function requestAwikiProfileUpgrade(
  ...
  const command = `dsh plugin --profile ${JSON.stringify(profileName)} add @awiki/dsh-plugin@${fallback.pluginVersion} @awiki/dsh-model-proxy@${fallback.modelProxyVersion}`
  const copy = locale === 'zh'
    ? {
        title: '需要升级 AWiki 插件',
        message: '当前 Profile 中的 AWiki 插件版本不兼容，继续启动可能导致功能异常。',
        detail: [
          'DSH Desktop 不会自动修改 Profile。请在终端运行以下精确命令，然后重启 DSH：',
          '',
          command,
        ].join('\n'),
    ...
    : {
        ...
        detail: [
          'DSH Desktop will not modify the Profile automatically. Run this exact command in the terminal, then restart DSH:',
          '',
          command,
        ].join('\n'),
