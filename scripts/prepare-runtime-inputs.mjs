...
export function validateManifest(manifest) {
  const test = manifest?.schemaVersion === 2 && manifest.channel === 'singapore-test' && manifest.published === false
  if ((!test && manifest?.schemaVersion !== 1) || !/^[a-f0-9]{64}$/u.test(manifest.archive?.sha256 ?? '')
    || !Number.isSafeInteger(manifest.archive?.sizeBytes) || manifest.archive.sizeBytes <= 0
    || !Array.isArray(manifest.packages) || manifest.packages.length !== (test ? 15 : 9)) {
    throw new Error('invalid Desktop runtime input manifest')
  }
  const url = new URL(manifest.archive.url)
  if ((test ? (url.origin !== 'https://anpclaw.com' || !/^\/downloads\/singapore-full-20260922\/plugins\/runtime-inputs-[a-f0-9]{16}\.tar\.gz$/u.test(url.pathname)) : (url.origin !== 'https://github.com'
  ...
  for (const pkg of manifest.packages) {
    if (!/^(?:@awiki|@agent-network-protocol)\/[a-z0-9-]+$/u.test(pkg.name)
      || !(test ? /^\d+\.\d+\.\d+-sg\.20260922\.[1-9]\d*$/u : /^\d+\.\d+\.\d+$/u).test(pkg.version)
...
export async function prepare({ verifyOnly = false } = {}) {
  ...
  try {
    ...
    console.log(`Desktop runtime inputs: all ${manifest.packages.length} package checksums verified`)
    return
  } catch (error) {
  ...
  try {
    ...
    console.log(`Desktop runtime inputs: downloaded and verified all ${manifest.packages.length} packages`)
  } finally {
